...
class Invoices extends Admin_Controller
{
    ...
    {
        if($jns!=null)
            redirect('invoices/status/'.$jns);
        else
            redirect('invoices/status/out-all');
     ...
     * @param int $page
     */
    public function status($status = 'out-all', $page = 0)
    {
        // Determine which group of invoices to load
        list($jns,$status)=explode('-',$status);
        switch ($status) {
            case 'draft':
     ...
     * @param $invoice_id
     */
    public function view($invoice_id,$idpo=null)
    {
        $this->load->model(
            array(
                'mdl_items',
    ...
    public function delete($invoice_id)
    {
        ...
        $po_id=$invoice->po_id;
        // $porder=$this->db->from('ip_po')->where('po_id',$po_id)->get()->row();
        $upd['po_status']=0;
        $this->db->where('po_id',$po_id);
        $this->db->update('ip_po',$upd);
