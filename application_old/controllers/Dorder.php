...
class Dorder extends Admin_Controller
{
     ...
     public function printcetak($invoice_id, $stream = true, $quote_template = null)
    {
        ...
        if($d[0]->status_invoices=='in')
        {
            // $this->load->helper('download');
            if(strpos($d[0]->do_file,'pdf')!==false)
            {
                $data   = file_get_contents('./uploads/import/'.$d[0]->do_file);
                $name   = $d[0]->do_file;
                $this->output
                    ->set_content_type('application/pdf')
                    ->set_output(file_get_contents('./uploads/import/'.$d[0]->do_file));
            }
            else
            {
                echo '<img src="'.base_url().'uploads/import/'.$d[0]->do_file.'">';
            }
        }
        else
        {
    ...
    {
        $thn=date('Y');
        $po=$this->db->query('select * from ip_invoices where YEAR(invoice_date_created)='.$thn.' and status_invoices="out" order by invoice_id desc limit 1')->result();
        $num=0;
