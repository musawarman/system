                            ...
                            <option value="<?php echo $invoice->invoice_id; ?>"
                                ...
                                <?php 
                                    if($invoice->vendor_id!=0)
                                    {
                                        ...
                                        echo ucwords($invoice->status_invoices).' - '.$invoice->invoice_number . ' - ' . format_vendor($vendor) . ' - ' . format_currency($invoice->invoice_balance);
                                    }
                                    else
                                        echo ucwords($invoice->status_invoices).' - '.$invoice->invoice_number . ' - ' . format_client($invoice) . ' - ' . format_currency($invoice->invoice_balance); 
                                
                                ?>
                            </option>
                        <?php } ?>
                    <?php } else { ?>
                        <option value="<?php echo $payment->invoice_id; ?>" selected="selected">
                            <?php
                                $inv=$this->db->from('ip_invoices')->where('invoice_id',$payment->invoice_id)->get()->row(); 
                                // echo $payment->invoice_number . ' - ' . format_client($payment) . ' - ' . format_currency($payment->invoice_balance); 
                                    if($inv->vendor_id!=0)
                                    {
                                        $invv=$this->db->from('ip_invoice_amounts')->where('invoice_id',$payment->invoice_id)->get()->row();
                                        $vendor=$this->db->from('ip_vendors')->where('vendor_id',$inv->vendor_id)->get()->row();
                                        echo ucwords($inv->status_invoices).' - '.$inv->invoice_number . ' - ' . format_vendor($vendor) . ' - ' . format_currency($invv->invoice_balance);
                                    }
                                    else
                                        echo ucwords($inv->status_invoices).' - '.$payment->invoice_number . ' - ' . format_client($payment) . ' - ' . format_currency($payment->invoice_balance); 
                            ?>
                        </option>
                    <?php } ?>
                </select>
